import { reverse, upcase } from '../../contracts/words';
...
describe("CONTRACT: Words", () => {
    ...
    it("should append to the log when reversing a word", () => {
        reverse(word)
        // log(VM.logs())
        expect(VM.logs()).toContainEqual("[" + alice + "] invoked function reverse(" + text + ")")
    });
    ...
    it("should append to the log when upcasing a word", () => {
        upcase(word)
        // log(VM.logs())
        expect(VM.logs()).toContainEqual("[" + alice + "] invoked function upcase(" + text + ")")
    });
